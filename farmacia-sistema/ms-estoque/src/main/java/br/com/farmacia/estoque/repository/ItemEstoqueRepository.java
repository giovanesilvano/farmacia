...
public class ItemEstoqueRepository {
    ...
    private String arquivo() {
        return path + "estoque.json";
    }

    public List<ItemEstoque> findAll() {
        return JsonFileHandler.lerArquivo(arquivo(), ItemEstoque[].class);
    }

    public List<ItemEstoque> findByProdutoId(Long produtoId) {
        return findAll().stream().filter(i -> i.getProdutoId().equals(produtoId)).collect(Collectors.toList());
    }

    public Optional<ItemEstoque> findById(Long id) {
