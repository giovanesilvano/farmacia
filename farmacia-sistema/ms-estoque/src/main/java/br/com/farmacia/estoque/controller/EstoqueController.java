...
public class EstoqueController {
    ...
    @PostMapping("/entrada")
    public ResponseEntity<ItemEstoque> entrada(@RequestBody ItemEstoque item) {
        return ResponseEntity.ok(service.registrarEntrada(item));
    }
    ...
    public ResponseEntity<Integer> saldo(@PathVariable Long produtoId) {
        return ResponseEntity.ok(service.consultarSaldo(produtoId));
    }

    @GetMapping("/historico")
    public ResponseEntity<List<MovimentacaoEstoque>> historico() {
        return ResponseEntity.ok(service.consultarHistorico());
    }

    @GetMapping("/alertas")
    public ResponseEntity<List<Produto>> alertas() {
        return ResponseEntity.ok(service.verificarAlertaMinimo());
    }
}
